    ...
    def process_frame(self, frame):
        ...
        for box, cls in zip(object_results[0].boxes.xyxy.cpu().numpy(), 
            ...
            if cls == 0:  # COCO class 0 is person
                cv2.putText(annotated_frame, "Person detected - No license plate", 
                          (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 
                          0.9, (0, 0, 255), 2)
                self.gate_controller.close_gate()
                return annotated_frame, "Person", False
        
        # Step 2: If no person, proceed with license plate recognition
    ...
    def process_detection(self, video_source):
        ...
        while attempt < self.max_capture_attempts and not authorized:
            ...
            log_entry = {
                ...
                'detected_object': plate_text if isinstance(plate_text, str) else "Vehicle",
                'plate_text': plate_text if plate_text not in ["Person", "No license plate detected"] else "None",
            ...
            print(f"Detection result: {plate_text}")
