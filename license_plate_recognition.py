...
class LicensePlateSystem:
    ...
    def check_authorization(self, plate_text):
        """Check if plate is authorized in database"""
        try:
            response = requests.get(
                f"http://localhost:5000/check_plate?plate={plate_text}",
                timeout=3
            )
    ...
    def process_frame(self, frame):
        ...
        if vehicle_detected and not authorized:
            self.gate_controller.close_gate()
    ...
    def process_video(self, video_path, output_dir='output', frame_skip=3):
        ...
        # Ensure gate is closed when done
        self.gate_controller.close_gate()
